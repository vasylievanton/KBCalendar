package kingbird.ru.rangecalendar.customviews;
...
import kingbird.ru.rangecalendar.R;
import kingbird.ru.rangecalendar.models.CalendarStyleAttr;
import kingbird.ru.rangecalendar.models.DayContainer;
...
class DateRangeMonthView extends LinearLayout {
    ...
    private void drawCalendarForMonth(Calendar month) {
        ...
        String[] weekTitle = DateFormatSymbols.getInstance(Locale.getDefault()).getShortWeekdays();
        List<String> daysName = new ArrayList<>(Arrays.asList(weekTitle));
        daysName.remove(0);
        weekTitle = new String[daysName.size()];
        daysName.toArray(weekTitle);
        ...
        for (int i = 0; i < 7; i++) {
            ...
            textView.setText(weekStr.toUpperCase());
    ...
    private void makeAsSelectedDate(DayContainer container, @DateRangeCalendarManager.RANGE_TYPE int stripType) {
        ...
        Drawable mDrawable = ContextCompat.getDrawable(getContext(), R.drawable.select_date_rect);
